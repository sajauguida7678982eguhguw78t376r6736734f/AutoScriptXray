...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/givpn/AutoScriptXray/master > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/givpn/AutoScriptXray/master | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS iplowq.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/givpn/AutoScriptXray/master | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
wget -q -O /usr/bin/proxy3.js "https://raw.githubusercontent.com/givpn/izin/master/AutoScriptXray/ssh/proxy3.js"
...
wget -q -O /usr/bin/ssh-wsenabler "https://raw.githubusercontent.com/givpn/izin/master/AutoScriptXray/ssh/sshws-true.sh" && chmod +x /usr/bin/ssh-wsenabler && /usr/bin/ssh-wsenabler
